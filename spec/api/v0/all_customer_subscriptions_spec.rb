...
RSpec.describe 'all customer subscriptions' do
  ...
  describe 'happy path' do
    it 'can get all customer subscriptions' do
      ...
      expect(response_body[:data][0][:attributes][:customer_id]).to eq(@customer1.id)
      ...
      expect(response_body[:data][3][:id]).to eq(@subscription4.id.to_s)
      ...
      expect(response_body[:data][3][:attributes][:title]).to eq('Herbal Tea Subscription')

      expect(response_body[:data][3][:attributes]).to have_key(:price)
      expect(response_body[:data][3][:attributes][:price]).to be_a(Float)
      expect(response_body[:data][3][:attributes][:price]).to eq(10.51)
      ...
      expect(response_body[:data][3][:attributes][:customer_id]).to eq(@customer1.id)

      expect(response_body[:data][3][:attributes]).to have_key(:tea_id)
      expect(response_body[:data][3][:attributes][:tea_id]).to be_a(Integer)
      expect(response_body[:data][3][:attributes][:tea_id]).to eq(@tea4.id)
    end
  end
  ...
  describe 'sad path' do
    it 'returns 404 if customer is not found' do
      ...
      expect(response_body).to be_a(Hash)
      expect(response_body[:errors][:detail]).to eq("Couldn't find Customer with 'id'=0")
    end
  end
end